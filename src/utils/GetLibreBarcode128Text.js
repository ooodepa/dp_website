...
var CodeSet = (function () {
  ...
  _p.get = function (value) {
    ...
    if (!symbol) {
      ...
      if (this.name === 'A') isCtrl = checksumValue >= 64;
      else if (this.name === 'B') isCtrl = checksumValue >= 95;
      else if (this.name === 'C') isCtrl = checksumValue >= 100;
...
function _addPaths(weight2Paths, paths) {
  var i, l, encoded, equalWeightPaths;
...
Object.defineProperties(_p, {
  ...
  chars: {
    get: function () {
      if (!this._chars) {
        var allChars = this.symbols.map(function (s) {
          return s.char;
        });
        allChars.push(
          this._getCheckSumSymbol().char,
...
export default function GetLibreBarcode128Text(code) {
  return encode(`${code}`);
}
